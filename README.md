...
* **An environment:** a task that determine the agents' immediate reward $r(\cdot |s,a)$ and the physical or logical dynamics (transition function $P\_t (\cdot|s,a)$)
* **<span style="color:orange;">The disruptor module</span>span>:** represents the uncertainty/perturbation events that happens during any parts of the interaction process between the agent and environment. 
