...
from robust_gymnasium import Space, error, logger, spaces
...
def check_space(
    ...
    if not isinstance(space, spaces.Space):
        if str(space.__class__.__base__) == "<class 'gym.spaces.space.Space'>":
            raise TypeError(
                f"Gym is incompatible with robust_gymnasium, please update the environment {space_type}_space to `{str(space.__class__.__base__).replace('gym', 'robust_gymnasium')}`."
            )
        else:
            raise TypeError(
                f"{space_type} space does not inherit from `robust_gymnasium.spaces.Space`, actual type: {type(space)}"
            )
...
def env_reset_passive_checker(env, **kwargs):
    ...
    if "seed" not in signature.parameters and "kwargs" not in signature.parameters:
        logger.deprecation(
            "Current robust_gymnasium version requires that `Env.reset` can be passed a `seed` instead of using `Env.seed` for resetting the environment random number generator."
        )
    else:
    ...
    if "options" not in signature.parameters and "kwargs" not in signature.parameters:
        logger.deprecation(
            "Current robust_gymnasium version requires that `Env.reset` can be passed `options` to allow the environment initialisation to be passed additional information."
        )
