...
from robust_gymnasium.error import CustomSpaceError
...
from robust_gymnasium.spaces.space import T_cov
...
def batch_space(space: Space[Any], n: int = 1) -> Space[Any]:
    ...
    Example:
        ...
        >>> from robust_gymnasium.spaces import Box, Dict
        >>> import numpy as np
        >>> space = Dict({
    ...
    """
    raise TypeError(
        f"The space provided to `batch_space` is not a robust_gymnasium Space instance, type: {type(space)}, {space}"
    )
...
def iterate(space: Space[T_cov], items: Iterable[T_cov]) -> Iterator:
    ...
    Raises:
        ValueError: Space is not an instance of :class:`robust_gymnasium.Space`
    ...
    if isinstance(space, Space):
        ...
        raise TypeError(
            f"The space provided to `iterate` is not a robust_gymnasium Space instance, type: {type(space)}, {space}"
        )
...
def concatenate(
    ...
    """
    raise TypeError(
        f"The space provided to `concatenate` is not a robust_gymnasium Space instance, type: {type(space)}, {space}"
    )
...
def create_empty_array(
    ...
    Raises:
        ValueError: Space is not a valid :class:`robust_gymnasium.Space` instance
    ...
    """
    raise TypeError(
        f"The space provided to `create_empty_array` is not a robust_gymnasium Space instance, type: {type(space)}, {space}"
    )
