...
class Dict(Space[typing.Dict[str, Any]], typing.Mapping[str, Space[Any]]):
    ...
    Example:
        >>> from robust_gymnasium.spaces import Dict, Box, Discrete
        ...
        >>> from robust_gymnasium.spaces import Box, Dict, Discrete, MultiBinary, MultiDiscrete
        >>> Dict(  # doctest: +SKIP
        ...     {
    ...
    convert :class:`Dict` observations to flat arrays by using a :class:`robust_gymnasium.wrappers.FlattenObservation` wrapper.
    ...
    def __setitem__(self, key: str, value: Space[Any]):
        ...
        assert isinstance(
            value, Space
        ), f"Trying to set {key} to Dict space with value that is not a robust_gymnasium space, actual type: {type(value)}"
