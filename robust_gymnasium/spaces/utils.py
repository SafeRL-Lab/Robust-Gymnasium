...
from robust_gymnasium.spaces import (
    Box,
    Dict,
    Discrete,
    Graph,
...
def flatdim(space: Space[Any]) -> int:
    ...
    Raises:
         ...
         ValueError: if the space cannot be flattened into a :class:`robust_gymnasium.spaces.Box`

    Example:
        >>> from robust_gymnasium.spaces import Dict, Discrete
...
def flatten(space: Space[T], x: T) -> FlatType:
    ...
    Returns:
        The flattened datapoint
            ...
            - For :class:`robust_gymnasium.spaces.Box` and :class:`robust_gymnasium.spaces.MultiBinary`, this is a flattened array
            - For :class:`robust_gymnasium.spaces.Discrete` and :class:`robust_gymnasium.spaces.MultiDiscrete`, this is a flattened one-hot array of the sample
            - For :class:`robust_gymnasium.spaces.Tuple` and :class:`robust_gymnasium.spaces.Dict`, this is a concatenated array the subspaces (does not support graph subspaces)
    ...
    Raises:
        NotImplementedError: If the space is not defined in :mod:`robust_gymnasium.spaces`.

    Example:
        >>> from robust_gymnasium.spaces import Box, Discrete, Tuple
...
def flatten_space(space: Space[Any]) -> Box | Dict | Sequence | Tuple | Graph:
    ...
    This function will attempt to flatten ``space`` into a single :class:`robust_gymnasium.spaces.Box` space.
    However, this might not be possible when ``space`` is an instance of :class:`robust_gymnasium.spaces.Graph`,
    :class:`robust_gymnasium.spaces.Sequence` or a compound space that contains a :class:`robust_gymnasium.spaces.Graph`
    or :class:`robust_gymnasium.spaces.Sequence` space.
    This is equivalent to :func:`flatten`, but operates on the space itself. The
    result for non-graph spaces is always a :class:`robust_gymnasium.spaces.Box` with flat boundaries. While
    the result for graph spaces is always a :class:`robust_gymnasium.spaces.Graph` with
    ...
    Example - Flatten spaces.Discrete:
        >>> from robust_gymnasium.spaces import Discrete
        >>> discrete = Discrete(5)
    ...
    Example - Flatten spaces.Dict:
        >>> from robust_gymnasium.spaces import Dict, Discrete, Box
    ...
    Example - Flatten spaces.Graph:
        >>> from robust_gymnasium.spaces import Graph, Discrete, Box
